...
import { cleanDatabase, seedTestDatabase, seedTestDatabaseUserOnly } from '../../helpers/database-helper.js';
...
describe("Client Endpoints - Integration Tests",()=>{
	...
	describe("POST /oauth/clients",()=>{
		it("should create a new OAuth client with valid data",async ()=>{
			...
			expect(response.body.client.redirectUris).toEqual(clientData.redirectUris);
	...
	describe("GET /oauth/clients",()=>{
		it('should list all clients for authenticated user', async () => {
			...
			// Verify client data
			const returnedClient1 = response.body.clients.find((c: any) => c.clientId === 'test-client-1');
		...
		it('should return empty array when user has no clients', async () => {
			// Note: Using seedTestDatabaseUserOnly for this test to start with clean clients
			...
			const testCookies = (loginResponse.headers['set-cookie'] as unknown as string[]) ?? [];
		...
		it('should only return clients owned by the authenticated user', async () => {
			...
			const myAppClient = response.body.clients.find((c: any) => c.clientName === 'My App');
	...
	describe("PUT /oauth/clients/:id",()=>{
		...
		it('should update redirect URIs', async () => {
			...
			// Act
			const newUris = ['https://new1.com/callback', 'https://new2.com/callback'];
			const response = await request(app)
				.put(`/client/${client.clientId}`)
				.set('Cookie', authCookies)
				.send({
	...
	describe("POST /oauth/clients/:id/rotate-secret",()=>{
		...
		it('should require authentication', async () => {
			// Act & Assert
			await request(app)
				.post('/client/some-client/rotate-secret')
				.expect(401);
		});
